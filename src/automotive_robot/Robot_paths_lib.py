...
def get_safe_radius(skeleton_path, robot_vision):
    if len(skeleton_path) == 3:
        return robot_vision/2.5
    else:
        safe_radius = robot_vision / 2.5
...
def get_critical_ls(skeleton_path, traversal_sight, robot_vision):
    ...
    print("current safe radius: ", safe_radius)
...
def approximately_sp_ls(critical_ls, spt, gpt):
    ...
    if len(critical_ls) > 0:
        ...
        flag_asp.extend([True for k in range(len(path) - 2)])
        flag_asp.append(False)
        ...
        for j in range(1000):
            ...
            for i in range(1, len(path) - 1):
                ...
                #print("Error here: ", flag_asp, " len(path): ", len(path), " len(flag_asp)", len(flag_asp))
